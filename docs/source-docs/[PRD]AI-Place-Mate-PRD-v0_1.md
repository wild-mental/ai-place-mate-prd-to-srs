...
모든 Pain에 **실패 KPI**를 붙이고, 모든 AC에 **측정 가능한 임계치**를 넣었습니다. 기준선은 출처를 표시했습니다. `H` 가설(모의 인터뷰·여정 지도 추정), `E` 외부 공개 자료, `M` 직접 측정 가능(경쟁사 화면 관찰).
...
**기준선의 대부분이 가설입니다.** 인터뷰 열한 건은 모의이고 로그는 0건입니다. 이 문서의 목적은 정확한 목표값을 제시하는 것이 아니라 **무엇을 측정하면 틀렸음을 알 수 있는지**를 못 박는 것입니다. Phase 0 드라이런에서 기준선을 `M` 으로 전환한 뒤 목표값을 재설정합니다.
...
| Pain | 문제 | 실패 KPI (현행) | 기준선 | 출처 | Needs |
| --- | --- | --- | --- | --- | --- |
| **C2-1** | 인당 가격이 어디에도 표기되지 않음 | 예상 지출과 실제 결제액의 **오차율** | 15% | `H` | 목록 단계에서 인당 예상가를 오차 범위와 함께 |
| **C2-2** | 무한리필류 운영 조건이 검색어로 없음 | 플랫폼 내 조건 검색 **성공률** | 0% | `M` | 조건을 카테고리로 한정해 즉시 필터 |
| **C3-1** | 색인이 가게 단위라 메뉴명으로 진입 불가 | 메뉴명 질의의 **정답 반환률** | 0% | `M` | 메뉴명 한 번으로 취급 매장 목록 |
| **C3-2** | 우회 탐색이 여러 채널로 분산 | 세션당 **외부 채널 수** / 소요 시간 | 4개 / 12분 | `H` | 1분 안에 첫 결과, 채널 하나로 완결 |
| **C4-1** | 문장형 조건을 넣을 입력 칸이 없음 | 상황어 질의 **처리율** | 0% | `M` | 머릿속 문장을 그대로 입력 |
| **C4-2** | 1인석·소음·체류 허용을 사전 확인 불가 | 방문 후 **조건 불일치 발생률** | 35% | `H` | 사진과 1인석 개수로 판단 재료 확보 |
| **C1-3** | 정한 곳을 정당화할 근거가 없음 | 후보 카드의 **선정 근거 표기율** | 0% | `M` | 선정 이유 한 줄 + 공유 가능한 형태 |
| **A2-2** | 룸·주차·법인카드·단체 수용이 필터에 없음 | 단체 조건 확인에 드는 **전화 통화 수** | 3통 / 15분 | `H` | 단체 조건을 한 번에 필터 |
| **N1-1·2** | 추천에 선정 근거가 없어 광고로 읽음 | 추천 결과 **신뢰 응답률** | 미측정 | `H` | 왜 이 집인지 문장으로 설명 |
| **P5-1** | 빈 시간대에 먼저 제안할 창구가 없음 | 빈 좌석 **공석률** / 고정 광고비 | 37% / 월 30만원 | `H` | 내가 조건과 폭을 정해 손님을 부르기 |
| **P5-3** | 노쇼로 좌석 회전과 준비가 이중 손실 | 단체 예약 **노쇼율** | 15% | `H` | 예약이 보증되는 구조 |
...
| 구분 | KPI | 정의 · 측정 방법 | 기준선 | 12주 목표 | 측정 주기 | 연결 기능 |
| --- | --- | --- | --- | --- | --- | --- |
| **북극성** | **주간 근거 기반 결정 완료 수**<br>WEBD · Weekly Evidence-Backed Decisions | 조건 2개 이상 입력 → Top-3 수신 → 1곳 선택(예약 또는 저장)까지 완주한 세션 수 | 0<br>미출시 | 5,000건/주 | 주간 | F1–F7 전체 |
| 보조 1 | 조건 입력 완료율 | 입력 시작 → 조건 2개 이상 확정 비율 | — | 70% | 주간 | F4 · F2 |
| 보조 2 | 첫 결과 p95 응답 시간 | 질의 수신 → Top-3 렌더 완료 | — | ≤ 1,000ms | 일간 | F1 · F6 |
| 보조 3 | Top-3 내 선택률 | Top-3 수신 세션 중 1곳을 선택한 비율 | — | 45% | 주간 | F6 · F5 |
| 보조 4 | 근거 표기율 | 선정 이유 1줄 + 확인 일자를 모두 가진 후보 카드 비율 | 0% `M` | 100% | 일간 | F5 |
| 보조 5 | 예상가 오차 ±10% 내 비율 | 표기 인당 예상가 대비 실제 결제액 편차 | 40% `H` | 80% | 월간 | F2 |
| 보조 6 | 방문 후 조건 불일치 신고율 | 방문 후 ‘조건이 달랐다’ 신고 / 방문 건수 | 35% `H` | ≤ 10% | 월간 | F1 · F5 |
| 보조 7 | 공유 카드 생성률 | 결정 세션 중 근거 카드를 외부로 공유한 비율 | — | 60% | 주간 | F5 |
| 보조 8 | 3분 내 제안 도착률 | 대화방 형성 → 180초 내 유효 제안 1건 이상 | — | 85% | 일간 | F7 · F8 |
| 보조 9 | 제안 이행률 | 현장에서 제안 내용이 그대로 적용된 비율 | — | ≥ 95% | 월간 | F7 · F9 |
| 보조 10 | 선택 제안 노쇼율 | 선택된 제안의 예약 중 미방문 비율 | 15% `H` | ≤ 5% | 월간 | F9 |
| 보조 11 | 가맹점 월 제안 참여율 | 활성 가맹점 중 월 1회 이상 제안한 비율 | — | 60% | 월간 | F8 |
...
- **AC2** — **Given** 메뉴 색인에 해당 메뉴가 없고, **When** 유사 메뉴가 3건 이상 존재하면, **Then** ‘찾는 메뉴가 없어 유사 메뉴로 대체했다대체했다&rsquo>rsquo;를 명시하고 반환한다. `빈 결과 반환률 ≤ 2%`
...
- **AC2** — **Given** 어떤 속성이 90일 이상 갱신되지 않았고, **When** 그 속성이 후보 카드에 노출되면, **Then** ‘확인 90일 경과’ 경고를 함께 표시한다. `경고 누락률 0%` `판정형 문구 0건`
- **AC3** — **Given** 이용자가 후보를 하나 선택하고, **When** 공유를 요청하면, **Then** 선정 이유와 근거를 담은 카드가 **3초 내에** 이미지 또는 링크로 생성된다. `생성 p95 ≤ 3,000ms` `공유 카드 생성률 ≥ 60%`
- **AC4** — **Given** 방문이 완료되고, **When** 이용자가 ‘조건이 달랐다’를 신고하면, **Then** 해당 속성의 확인 상태가 ‘재확인 필요’로 즉시 전환된다. `반영 지연 ≤ 60s` `불일치 신고율 ≤ 10%`
...
- **AC3** — **Given** 이용자가 예약 2시간 전까지 취소하고, **When** 취소가 접수되면, **Then** 선결제 금액을 전액 환불하고 매장에 즉시 알린다. `환불 처리 ≤ 24h` `환불 실패율 ≤ 0.5%`
- **AC4** — **Given** 예약 시각이 지나고, **When** 방문 확인이 없으면, **Then** 노쇼로 기록하고 선결제 금액을 매장에 정산한다. `선택 제안 노쇼율 ≤ 5%` `오판정률 ≤ 1%`
...
- **AC2** — **Given** 등록한 수용 조건(인원·시간·룸 등) 밖의 요청이 도착하고, **When** 시스템이 매칭을 시도하면, **Then** 해당 매장의 에이전트는 소환되지 않는다. `부적합 소환률 ≤ 5%` `알림 무시율 ≤ 20%`
- **AC3** — **Given** 프로필 작성이 완료되고, **When** 사장이 내용을 갱신하려 하면, **Then** 기존 항목을 **1회 클릭**으로 불러와 수정할 수 있다. `프로필 갱신율 ≥ 70%` `월 제안 참여율 ≥ 60%`
- **AC4** — **Given** 사장이 강점 문구를 입력하고, **When** 등록된 사실 속성으로 뒷받침되지 않는 표현이 포함되면, **Then** 해당 문구를 저장하지 않고 근거가 될 속성 등록을 안내한다. `근거 없는 제안 문구 0건`
...
- **AC4** — **Given** 180초가 지나고, **When** 유효 제안이 0건이면, **Then** 빈 화면 대신 **제안 없는 Top-3**로 되돌리고 그 사실을 알린다. `빈 제안 화면 노출 0건`
- **AC5** — **Given** 제안을 선택해 방문하고, **When** 현장에서 제안 내용이 적용되지 않으면, **Then** 이용자 신고로 불이행이 기록되고 해당 매장의 소환 가중치가 하향된다. `제안 이행률 ≥ 95%` `신고 처리 ≤ 48h`
...
| 코드 | 기능 | MSCW | Phase | 대안 대비 가치 | KPI | 우선순위 근거 |
| --- | --- | --- | --- | --- | --- | --- |
| **F1** | 속성·dish 단위 공통 색인 | Must | P1 | 조건 검색 성공률 **0% → 90%** | 보조 2·6 | 나머지 여덟 개가 이 위에 얹힙니다. 색인 단위는 스키마 결정이라 **나중에 바꾸면 전면 재색인** |
| **F2** | 인당 가격대 · 메뉴 옵션 필터 | Must | P1 | 가격 확인 노동 **4.2분 → 0초** | 보조 5 | DOS 1·2위. 대중점평이 인당 평균가 규격화로 **수익성을 검증**한 축 |
| **F3** | 대표 메뉴 단위 추천 | Must | P1 | 메뉴 질의 정답률 **0% → 92%** | 보조 2·3 | DOS 3위. 점심 일상에서 매일 발생하고 **대체 수단이 존재하지 않음** |
| **F4** | 자연어 상황 검색 | Must | P1 | 탐색 채널 **4개 → 1개** | 보조 1 | F1의 속성이 사용자에게 닿는 유일한 통로. 파싱 실패 시 **폴백 필터로 열화** |
| **F5** | 근거·출처 표기 + 공유 카드 | Must | P1 | 근거 표기율 **0% → 100%** | 보조 4·7 | 저난이도 × 고효과. C1·C4·E1·N1 **네 페르소나가 독립적으로 같은 요구** |
| **F6** | 근거 있는 Top-3 제시 | Must | P1 | 결정 시간 **−20%** · 비교 축 정리 | 보조 3 · NS | 후보를 몇 개로 보여 줄지는 **화면 설계 선택**이지 차별 축이 아닙니다. 값은 **비교 축과 근거를 미리 정리**해 고민할 양을 줄이는 데서 나옵니다 |
| **F9** | 선택과 예약 · 결제 | Should | P1 말 | 재입력 필드 **N개 → 0개** · 노쇼 **15% → 5%** | 보조 10 | **F7·F8의 선행 조건.** 제안에 인원·메뉴가 담겨 있어 조건 재입력 없이 결제로 이어집니다. 선결제가 노쇼 손실을 흡수 — OpenTable이 노쇼 관리의 판매 가능성을 이미 증명 |
| **F8** | 매장 에이전트 콘솔 | Could | P2 | 고정 광고비 **월 30만 → 0원** | 보조 11 | **할인폭이 아니라 강점·서비스·수용 조건**을 등록하는 콘솔. 사장이 직접 조작하므로 **화면 수가 곧 이탈률**이라 설정 화면 3개 이하 |
| **F7** | 에이전트 소환 · 단체 대화방 | Could | P2 | 매장 에이전트 제안 **부재 → 상용** | 보조 8·9 | 사업 모델의 축이자 **경쟁사 일곱 곳 전부 부재**인 유일한 축. F1·F6·F8·F9가 모두 선행 |
| **F1a** | 알레르기·성분 필드 | Won't | 필드만 | — | — | F1과 같은 자료구조라 **스키마 자리만 확보**. 등록 정보를 그대로 노출하고 판정하지 않음 |
| **F1b** | 접근성 속성 필드 | Won't | 필드만 | — | — | 입구 사진·단차 필드만. **검증되지 않은 구간은 명시**하고 정확도를 약속하지 않음 |
...
> - **공급 측** 매장 에이전트 콘솔(분위기·강점·서비스·수용 조건), 에이전트 소환과 제안 수집, 주문량 기반 결제 — **Phase 2**
...
>
> P5의 **‘대화방에서 자기 강점을 제안할 의향’이 모의값**입니다. 매장이 프로필을 채우지 않거나 제안하지 않으면 F7·F8과 차별 가치 03이 함께 무너지고, 대화방이 **참여 매장 없는 빈 화면**이 됩니다.
>
...
> 가정 (Assumptions)
>
> - **A1** 인당 예산 상한을 두는 이용자가 수도권 20–34세에서 과반이다 `미검증`
> - **A2** 매장은 자기 강점·서비스를 대화방에서 제안할 의향이 있다 `미검증 · R1`
> - **A3** 후보 3개는 목록 20개보다 선택률이 높다 `E3로 검증`
> - **A4** 근거 표기가 선택률과 재방문을 올린다 `E2로 검증`
> - **A5** 결제 데이터로 인당가를 ±20% 내로 추정할 수 있다 `미검증 · R2`

> 의존성 (Dependencies) · ADR
>
> - **ADR-001** 색인 단위를 place가 아니라 **dish + attribute**로 결정 — 되돌리면 전면 재색인
> - **ADR-002** Verification을 **별도 엔터티**로 분리 — F5의 전제
> - **ADR-003** 후보 수를 **3개로 고정** 페이지네이션 미제공
> - **ADR-004** v0.1은 광고 상품 **미도입** 차별 가치의 전제(KSF 02)
> - **외부** PG 계약(주문량 기반 결제) · 가맹 온보딩 인력 · 상권 3곳 데이터 확보
...
> - **기준선 갱신** Phase 0 종료 시 `H` 기준선을 전량 `M`으로 교체하고 목표값 재설정
...
두 방향으로 나눴습니다. 왼쪽은 **이미 확보한 근거**(인터뷰·벤치마크·시장 통계), 오른쪽은 **출시 후 세울 측정 체계**(행동 감지·통계 추출·KPI·실험). 왼쪽의 대부분이 `H` 가설이므로, 이 문서의 실질 산출물은 오른쪽입니다.

### 방향 1 확보한 근거

*가설 · 외부 자료*

> *인터뷰***모의 JTBD 11건**실제 인터뷰 0건
>
...
> - **P5** “광고 영업 오면 저는 듣기만 하잖아요. 이건 내가 조건을 거는 거네요” → F8의 자기 소개 콘솔. **에이전트 제안 모델이 이 한 발화에 걸려 있고 모의값**
...
| --- | --- | --- | --- | --- |
| **4.2분** | 인당 가격을 확인하는 데 드는 시간 | 가설 | 리뷰·블로그를 거치는 탐색 단계 수에서 역산 | PRD v0.1 §1.3 보조 5 |
| **정답률 92%** | dish 질의에 정답을 돌려주는 비율 | 목표 기준선 0% | 메뉴명 정규화 키를 적용한 뒤의 목표치 | PRD v0.1 §3 US-3 · §6 |
| **노쇼율 15% → 5%** | 단체 예약이 방문으로 이어지지 않는 비율 | 15% 가설 5% 목표 | 선결제가 노쇼를 흡수한다는 가정 | PRD v0.1 §1.3 · §3 US-4 AC4 |
| **WEBD 5,000건/주** | 근거를 보고 결정을 끝낸 주간 건수 | 목표 | 북극성 KPI 정의 | PRD v0.1 §1.3 |
| **첫 결과 p95 ≤ 1,000ms** | Top-3가 화면에 도착하는 시간 | 목표 | NFR 성능 예산. 롤아웃 게이트는 별도로 1.5s를 씁니다 | PRD v0.1 §5 |

> #### 인용 규칙
>
> 다섯 값은 전부 **PRD가 만들었습니다.** JTBD·CJM·AOS·DOS에는 원래 없는 수입니다. 다른 덱에서 쓸 때는 PRD 출처를 함께 적고, 그 덱이 관찰한 결과처럼 쓰지 않습니다. 인터뷰와 세션 로그가 쌓이기 전까지 상태 칸은 **가설과 목표**로 남습니다.
...
> 수를 **만드는 자리**와 **인용하는 자리**를 갈라 두는 것이 이 표의 목적입니다. 값이 바뀌면 PRD에서 고치고 나머지 덱은 인용만 갱신합니다. 가설을 관찰로 올리는 일은 인터뷰와 로그가 실제로 쌓인 뒤에 합니다.
