...
| 데이터 분석가 | 데이터팀 | 주간 리포트, 실험 설계·분석, 기준선 실측 |
...
> **근거:** ISO/IEC/IEEE 29148:2018 §9.6.19 Verification — 소프트웨어를 적격화하기 위해 계획된 **검증 접근법과 방법**을 제시하며, §9.6.10 ~ §9.6.18의 정보 항목과 **병렬로** 기술할 것을 권고한다. 본 장은 §4의 요구사항과 병렬로 성과 지표·실험·릴리스 게이트를 기술한다.
...
| ID | 검증 대상 가설 | 실험 설계 | 측정 도구 | 성공 기준 | Phase |
| --- | --- | --- | --- | --- | --- |
| **E1** | 자연어 단일창이 3필드 입력보다 완료율이 높다 | A/B n=500 세션, 50:50 무작위, 2주 | 이벤트 로그 — 완료율·p95·파싱 실패율 | 완료율 +10pp 이상 & p95 ≤ 1,000ms | P1 |
| **E2** | 근거 표기가 Top-3 내 선택률을 올린다 | A/B n=800 세션, 근거 유/무 2군, 3주 | 이벤트 로그 + 7일 재방문율 | 선택률 +8pp 이상 & 재방문 +5pp | P1 |
| **E3** | 후보 3개가 5개보다 선택률과 결정 시간에서 낫다 | A/B n=600 세션, 3개 vs 5개, 2주 | 이벤트 로그 — 선택률·결정 소요 시간 | 선택률 동등 이상 & 결정 시간 −20% | P1 |
| **E4** | 예상가를 범위로 표기하면 신뢰도가 올라간다 | A/B n=400, 단일값 vs 범위 + 방문 후 5점 설문 | 설문 + 결제액 편차 로그 | 신뢰도 ≥ 4.0/5 & ±10% 내 80% | P1 |
| **E5** | 선결제가 노쇼를 줄이되 전환을 크게 깎지 않는다 | A/B n=300 단체 제안, 0원 vs 선결제, 4주 | 예약·방문 매칭 로그 | 노쇼 ≤ 5% & 전환 하락 ≤ 5pp | P2 |
| **E6** | 제안 마감 180초가 90초보다 도착률이 높다 | A/B n=300 요청, 90s vs 180s, 3주 | 대화방·제안 로그 | 도착률 ≥ 85% & 대기 중 이탈 ≤ 15% | P2 |
| **E7** | 동일 과업에서 기존 대안보다 빠르다 | 벤치마크 — 동일 과업 10건을 경쟁 3사에서 수동 수행, 평가자 2명 교차, n=60 | 스톱워치 + 화면 녹화 | 소요 1/10 이하 & 채널 4→1 & 근거 표기 0→100% | P0–P1 |

**실험 운영 규칙**

- 동시 실행 실험은 **2개 이하** — 상권 3곳 규모에서 3개 이상은 표본이 갈려 유의성이 확보되지 않는다
- 노쇼 관련 실험은 릴리스 게이트와 연동한다 — E5에서 노쇼 8% 초과 시 즉시 중단하고 REQ-FUNC-009·008 노출을 정지한다
- Phase 0 종료 시 실측 기준선으로 §10.1 표를 갱신하고 목표값을 재보정한다
...
| LIM-11 | 동시 실행 A/B 실험을 2개 이하로 제한한다 | §9.6.7 i) | §10.2 |
...
| **R4** | 선결제가 예약 전환을 떨어뜨림 | 중대 · 中 | 경쟁 서비스가 '0원 예약'을 내세운 시장에서 선결제 요구가 전환율을 낮춘다 | 4인 이상 단체 제안에만 선결제 적용 · 취소 시 24시간 내 전액 환불 · E5로 노쇼율과 전환율의 교환비를 실측 | REQ-FUNC-007 · E5 |
...
| **ASM-03** | 후보 3개는 목록 20개보다 선택률이 높다 | E3 A/B (n=600) | REQ-FUNC-006의 후보 수를 5개로 조정하고 ADR-003 재검토 |
| **ASM-04** | 근거 표기가 선택률과 재방문을 올린다 | E2 A/B (n=800) | REQ-FUNC-005의 근거를 **접힘 기본**으로 변경해 렌더 비용 절감 |
...
2. **E7 벤치마크** — 대안 대비 배수 주장을 실증하거나 문서에서 내린다
...
| **ADR-003** | 후보 수를 **3개로 고정**하고 페이지네이션을 제공하지 않는다 | 경쟁사가 전부 '목록을 길게'로 경쟁하며 그 결과가 스크롤 길이를 늘리는 제품이다. 후보 수는 화면 설계 선택이라 되돌릴 수 있어야 한다 | 결정 시간 −20%가 목표이고, 후보를 늘리면 WEBD가 아니라 체류 시간이 올라간다 | ① 5개 + 더보기<br>② 20개 목록 — 둘 다 현행 경로와 같아져 차별 축이 소멸 | **낮음** — 렌더 파라미터 변경 수준 | E3 A/B (n=600) — 열위 시 5개로 조정 |
...
| §10.2 실험 · §10.4 게이트 | §8 실험 설계와 단계별 출시 |
| §10.3 관측 항목 | §5.5 모니터링 |
| §11.4 리스크 | §7.2 리스크 |
