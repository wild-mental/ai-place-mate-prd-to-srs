...
- [x] 태스크 축약 — 118 → 56건 (BE/UI 계층 분리 반영) → 미담당 요구사항 3건 승격 = **59건 · 커버리지 42/42**
- [x] 태스크 이슈 명세 59/59건 추출 (`docs/tasks/`)
...
| 항목 | 값 |
| --- | --- |
| DAG 레벨 | **15단계** (동시 착수 가능 묶음) |
| 임계 경로 | **63영업일** · 15단계 |
| 자원 제약 완료 | **72영업일** (2026-09-01 ~ 12-09) |
| 총 공수 | 215 person-day |
| 트랙 구성 | 6레인 — 플랫폼1 · 백엔드2 · 프론트1 · 디자인1 · QA1 |

### 포함 내용

- **실행 전략** — 4대 원칙(게이트 우선 · 계약 우선 · Mock 병행 · 디자인 선행), 트랙 구성, 착수 전 외부 블로커 5종, 게이트·중단 조건, 운영 규칙
- **의존성 구조 (DAG 3단계)** — Epic 수준 16노드 → **전체 태스크 59노드·120간선** → Phase별 4개 → 임계 경로. 15레벨 표·병목·병렬 구간 포함
- **일정** — 트랙별 Gantt(59건 전건 배치 · 임계 경로 `crit` 표기), Phase·게이트 Gantt, 주차별 착수표
- **리스크·진척 추적** — 일정 관점 리스크 6종, 추적 지표 5종, 운영 리듬

### 검증

일정을 그리기만 한 것이 아니라 계산 결과를 검사했습니다.
...
| 레인 내 시간 중복 | **0건** |
| 임계 경로 허위 간선 | **0건** |
| Gantt 배치 | 59/59건 · 중복 0 |
| **전체 DAG 노드** | **59/59 · 누락 0** |
| **전체 DAG 간선** | **120/120 · 허위 0** |
| Phase별 DAG 커버 | 59/59 · 간선 120/120 |
| mermaid 렌더 | **9/9 통과** |

레인 가동률은 플랫폼 72% · 백엔드 68/58% · 디자인 42% · 프론트 31% · QA 28%입니다.
...
| 안 | 인원 | 완료 | 평균 가동률 |
| --- | --- | --- | --- |
| 기본안 (PLAN-001) | 6명 | 72일 | 50% |
| **압축안** (PLAN-FAST-001) | **9명** | **63일** | 38% |
| 최대 병렬 (참고) | 16명 | 63일 | 21% |

**63일이 하한입니다** — 임계 경로가 63영업일이라 16명을 넣어도 같습니다. 압축안은 하한에 정확히 도달하는 **최소 인원**을 탐색한 결과입니다.

### 핵심 발견

- **압축이 통하는 구간은 W1~W6뿐** — 동시 작업이 최대 9건까지 오릅니다. W7 이후는 `RSV → MCH → AGR`이 순수 직렬이라 1~2건으로 떨어져 인원을 넣어도 대기만 늘어납니다.
- **인원 상시 유지가 불필요** — 주차별 투입 곡선을 계산해 W6 이후 디자인·플랫폼 철수, W7 이후 백엔드 축소를 권고했습니다.
- **프론트·QA는 늘리지 않았습니다** — 임계 경로를 제약하지 않아 증원이 순수 비용입니다.
- ⚠️ **온보딩 시간이 일정에 없습니다** — −9일은 추가 인원이 즉시 생산성을 낸다는 가정이며, 이 맥락을 아는 인력일 때만 성립합니다.

### 검증

선행-후행 역전 0 · 레인 중복 0 · **완료 63일 = 임계 경로 63일** · Gantt 59/59 · mermaid 렌더 통과.
